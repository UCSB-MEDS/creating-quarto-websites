...
From Positron's **Explorer** menu (left side menu option that looks like two overlapping files), click on the **New File...** button, ![](media/positron-new-file.png){alt="A file with a plus sign, +, overlaid on it" style="display: inline-block; margin: 0; position: relative; top: 0px; width:20px;height:20px;"}, (if you don't see it, hover your mouse over your repository directory name). Type `.nojekyll` then press **Enter / Return** to add it.
...
Positron's **Preview** button (which becomes available when you open any of your website `.qmd` files, e.g. `index.qmd`) spins up a preview in the Viewer pane *and* fully renders your website so that it's ready for distribution (e.g. pushing to GitHub to deploy using GitHub pages).  
...
#| fig-align: "center"
#| out-width: "100%"  
knitr::include_graphics("media/positron-stage-commit-push.png")
```
