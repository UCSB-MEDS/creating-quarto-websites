...
#| echo: false
#| fig-align: center
#| fig-cap: "My Quarto website after updating my landing page with the built-in `trestles` template, adding an image, links, and some content. (**Note:** using three dashes, `---`, creates a page divider, as seen between sections in the screenshot above.) Overall, a big improvement! However, our text is a bit squished in the center of the page -- we'll fix then in the next step."
...
As you get more comfortable with the Quarto framework and CSS for styling webpages (we'll learn more about this in a later workshop!), you may decide to build a completely custom webpage layout. You can checkout the [Quarto documentation](https://quarto.org/docs/output-formats/page-layout.html) to learn more.
:::
...
<!-- 2. Remove the floating table of contents that appears on any page with level 1-3 headers -- you're of course welcome to keep this here if you prefer, or adjust on a page-by-page basis -->
...
editor: visual
```
1. A newly-added navbar page. **Note:** The page name, as it appears in the navbar of your website, will be the same as whatever is listed in the `title` field in your file's yaml. For example, I have set `title: "All my favorite resources!"` in the yaml of `resources.qmd` -- this is how it will appear in my website's navbar. If you'd like the page name to appear differently in the navbar, use the `href` and `text` options (e.g. see how `index.qmd` is rendered as `Home` in my website's navbar).
