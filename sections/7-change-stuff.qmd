...
editor: visual
```
1. A newly-added navbar page. **Note** the page name, as it appears in the navbar of your website, will be the same as whatever is listed in the `title` field in your file's yaml. For example, the in the yaml of `resources.qmd`, I have set `title: "All my favorite resources!"`. You can change how the page name appears in your navbar by setting `href` to your `qmd` file name and `text` to however you want your text to appear (e.g. see how `index.qmd` is rendered as `Home` in my website's navbar).
