...
Positron's **Preview** button (which becomes available when you open any of your website `.qmd` files, e.g. `index.qmd`) both spins up a preview in the Viewer pane *and* fully renders your website so that it's ready for distribution (e.g. pushing to GitHub to deploy using GitHub pages). That said, I'm in the habit of always running `quarto render` in the Positron Terminal when I'm ready to push my changes to GitHub.
...
Run `quarto render` in the Terminal -- you may use the Terminal in Positron, RStudio, or a separate command line interface (just make sure you’re in the correct directory).
