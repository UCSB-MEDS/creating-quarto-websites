...
Nope! There are a number of R-based tools that make building websites and blogs fun and relatively easy, including the [`{blogdown}`](https://bookdown.org/yihui/blogdown/){target="_blank"} and [`{distill}`](https://rstudio.github.io/distill/website.html){target="_blank"} packages.
...
That said, Quarto has the data science community abuzz -- it's versatile, user-friendly, and looks pretty great out-of-the-box (while still being customizable). Importantly, while Quarto grew out of the R Markdown ecosystem, you don’t need to use R to use Quarto, as it supports multiple languages and workflows. There's also an ever-growing number of excellent resources (see [Mickaël Canouil](https://github.com/mcanouil){target="_blank"}'s [awesome-quarto](https://github.com/mcanouil/awesome-quarto){target="_blank"} as a starting point) to help you on your own Quarto journey.
